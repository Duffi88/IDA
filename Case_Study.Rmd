...
NATest(T15)
```
Keine Auffälligkeiten außer, dass Wareneingangsdatum falschen Datentyp (character) hat -> Umwandlung zu Datumsformat
```{r, message = FALSE}
...
NATest(T16)
```
Produktionsdatum hat falschen Datentyp (double), zusätzliche Origing Spalte, die das Ursprungsjahr angibt hat auch falschen Datentyp (character) -> Umwandlung beider zu Datumsformat 
```{r, message = FALSE}
...
Anschließend wird nun nach fehlerhaften Komponenten als auch nach Komponenten, die fehlerhafte Einzelteile besitzen, gefiltert, sodass am Ende eine gesamtheitliche Tabelle kritischer Komponenten erzeugt werden kann.
...
names(GesTab) <- c("ID_Sitze", "ID_Fahrzeug", "Komponente_Produktionsdatum", "Komponente_Herstellernummer", "Komponente_Werksnummer", "Fahrzeug_Produktionsdatum", "Fahrzeug_Herstellernummer", "Fahrzeug_Werksnummer", "Gemeinden", "Zulassung", "Postleitzahl", "Laengengrad", "Breitengrad")
...
```{r, message = FALSE}
ggplot(GesTab.new, aes(x = Laengengrad, y = Breitengrad))+
  geom_point()
...
```
Die meisten Austos wurden in (L:6,25 B:51),(L:7,5 B:51,5),(L:6,75 B:51,5),(L:8,5 B:52),(L:13,75 B:51),(L:12,5 B:51,25) zugelassen.
