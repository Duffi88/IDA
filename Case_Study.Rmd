...
ggplot(GesTab, aes(x = Längengrad, y = Breitengrad))+
  geom_point()
  #scale_fill_gradient(low ="#00BFC4", high = "#F8766D")

GesTabCount <- count(GesTab,Postleitzahl)%>%
  left_join(Geo, by ="Postleitzahl")%>%
  arrange(n)
...
ggplot(GesTabCount, aes(x = Längengrad, y = Breitengrad, colour = n, size = n, alpha = 0.3))+
  geom_point()+
